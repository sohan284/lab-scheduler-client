    ...
    return scheduledTasks.some((task) => {
      if (task.selectedMachine.includes(machine)) {
        ...
        const bookedEndTime = new Date(
          bookedStartTime.getTime() + durationInMinutes * 60000
        );
    ...
    <div className="max-w-[1200px] mx-auto mt-5 p-6 bg-white rounded-lg">
      ...
      <div className="mt-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <span
              className={`ml-2 text-xs ${
                selectedTimeSlots.length > 0 ? "text-blue-600" : "text-gray-500"
              }`}
            >
              ...
              <div className="flex-1 flex items-center justify-center">
                <div className="grid grid-cols-4 gap-3 place-items-center">
                  {timeSlots.map((slot, index) => {
                    const isDisabled = selectedMachine.some((machine) =>
                      isMachineBooked(machine, [slot])
                    );
                    return (
                      <div
                        key={index}
        ...
        <div className="flex items-center mt-2 mx-10 text-xs px-10">
          ...
          />
          <button className="ml-2 text-black px-3 py-1 rounded">
            +Add More
          </button>
        </div>
        <button className="mt-2 ml-10 bg-[#522C80] text-white px-4 py-2 rounded shadow hover:bg-[#673c9c] text-xs transition">
          SHARE
        </button>
      </div>
    </div>
